...
const ReportsPanel = ({ reports, resolveReport, dismissReport }) => {
  ...
  return (
    <div className="bg-gradient-to-br from-white to-gray-50/30 rounded-xl p-6 border border-gray-200/60 shadow-lg">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-gray-600 mt-1">Review and handle user reports</p>
        </div>
      ...
      <div className="space-y-3">
        {filtered.length === 0 ? (
          <div className="text-center py-12 bg-white rounded-lg border border-gray-200">
            <svg viewBox="0 0 24 24" className="w-14 h-14 text-gray-300 mx-auto mb-4" fill="none" xmlns="http://www.w3.org/2000/svg">
              <path d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
            </svg>
            <h3 className="text-lg font-semibold text-gray-900 mb-2">No Reports Found</h3>
            <p className="text-sm text-gray-500">There are no reports matching your filter criteria.</p>
          </div>
        ) : (
          filtered.map(r => (
            <div key={r._id} className="border border-gray-200 rounded-lg p-4 hover:border-blue-300 hover:shadow-md transition-all duration-200 bg-white">
              <div className="flex items-start justify-between mb-3">
                <div className="flex items-center gap-2.5">
                  {r.reportedBy?.avatar ? (
                    ...
                    <div className="w-9 h-9 rounded-full bg-gradient-to-br from-blue-500 to-purple-600 flex items-center justify-center text-white font-bold text-sm shadow-sm ring-2 ring-white">
                  ...
                  <div>
                    <p className="text-sm text-gray-900 leading-tight">
                      ...
                      <span className="text-gray-600 font-medium"> reported a post by </span>
                      <span className="font-bold">{r.postId?.author?.username || r.postId?.author?.name || 'Unknown User'}</span>
                    </p>
                    <p className="text-xs text-gray-500 font-medium mt-1">{formatLastActive(r.createdAt)}</p>
                  </div>
                </div>
              ...
              <div className="mb-3">
                ...
                {r.postId?.content && (
                  <div className="bg-blue-50/50 px-3 py-2 rounded-md border border-blue-100/50">
                    <p className="text-xs text-gray-500 font-semibold uppercase mb-1">REPORTED POST:</p>
                    <p className="text-sm text-gray-700 line-clamp-2 leading-relaxed">{r.postId.content}</p>
                  </div>
                )}
              </div>
              ...
              {(r.status === 'pending' || r.status === 'reviewing') && (
                <div className="flex items-center gap-2 pt-2">
                  ...
                  <button 
                    className="inline-flex items-center gap-1.5 px-3 py-2 bg-gradient-to-r from-gray-100 to-gray-200 text-gray-700 text-sm rounded-md hover:from-gray-200 hover:to-gray-300 transition-all font-semibold shadow-sm hover:shadow-md"
                  ...
                  <button 
                    ...
                    className="inline-flex items-center gap-1.5 px-3 py-2 bg-white border border-gray-300 text-gray-700 text-sm rounded-md hover:bg-gray-50 hover:border-gray-400 transition-all font-semibold shadow-sm hover:shadow-md"
                  >
                    <svg viewBox="0 0 24 24" className="w-4 h-4" fill="none" xmlns="http://www.w3.org/2000/svg">
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-hidden transform transition-all animate-slideUp">
            ...
            <div className="p-6 overflow-y-auto max-h-[calc(90vh-140px)]">
              ...
              <div className="bg-gray-50 rounded-xl p-4 mb-4">
                <h4 className="text-sm font-bold text-gray-700 uppercase mb-3">Reporter Information</h4>
                <div className="flex items-center gap-3">
                  {selectedReport.reportedBy?.avatar ? (
                    ...
                    <div className="w-12 h-12 rounded-full bg-gradient-to-br from-blue-500 to-purple-600 flex items-center justify-center text-white font-bold text-lg ring-2 ring-blue-200">
                      {selectedReport.reportedBy?.name?.charAt(0) || '?'}
                    </div>
                  )}
                  <div>
                    <p className="font-bold text-gray-900">{selectedReport.reportedBy?.name || 'Unknown User'}</p>
                    <p className="text-sm text-gray-600">@{selectedReport.reportedBy?.username || 'unknown'}</p>
                    <p className="text-xs text-gray-500">{selectedReport.reportedBy?.email || 'No email'}</p>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-red-50 rounded-xl p-4 mb-4">
                <h4 className="text-sm font-bold text-gray-700 uppercase mb-3">Reported Post Author</h4>
                <div className="flex items-center gap-3">
                  {selectedReport.postId?.author?.avatar ? (
                    ...
                    <div className="w-12 h-12 rounded-full bg-gradient-to-br from-red-500 to-orange-600 flex items-center justify-center text-white font-bold text-lg ring-2 ring-red-200">
                      {selectedReport.postId?.author?.name?.charAt(0) || '?'}
                    </div>
                  )}
                  <div>
                    <p className="font-bold text-gray-900">{selectedReport.postId?.author?.name || 'Unknown User'}</p>
                    <p className="text-sm text-gray-600">@{selectedReport.postId?.author?.username || 'unknown'}</p>
                    <p className="text-xs text-gray-500">{selectedReport.postId?.author?.email || 'No email'}</p>
                  </div>
                </div>
              </div>
              ...
              {/* Report Reason */}
              <div className="mb-4">
                <h4 className="text-sm font-bold text-gray-700 uppercase mb-2">Report Reason</h4>
                <div className="flex gap-2">
                  <span className="inline-flex items-center text-sm bg-gradient-to-r from-purple-50 to-purple-100 text-purple-700 px-4 py-2 rounded-xl font-bold shadow-sm">
                    Post Report
                  </span>
              ...
              {selectedReport.postId?.content && (
                <div className="mb-4">
                  <h4 className="text-sm font-bold text-gray-700 uppercase mb-2">Reported Post Content</h4>
                  <div className="bg-blue-50 p-4 rounded-xl border border-blue-200">
                    <p className="text-sm text-gray-700 leading-relaxed">{selectedReport.postId.content}</p>
                  </div>
                </div>
              )}
              ...
              {selectedReport.postId?.images && selectedReport.postId.images.length > 0 && (
                <div className="mb-4">
                  <h4 className="text-sm font-bold text-gray-700 uppercase mb-2">Post Images ({selectedReport.postId.images.length})</h4>
            ...
            <div className="p-6">
              <div className="bg-green-50 border border-green-200 rounded-xl p-4 mb-6">
                <div className="flex gap-3">
                  <svg viewBox="0 0 24 24" className="w-5 h-5 text-green-600 flex-shrink-0 mt-0.5" fill="none" xmlns="http://www.w3.org/2000/svg">
              ...
              <div className="bg-gray-50 rounded-xl p-4 mb-6">
                <p className="text-sm text-gray-700 mb-2">
                  <span className="font-bold">{selectedReport.reportedBy?.name || 'Unknown User'}</span>
                  <span className="text-gray-600"> reported a post by </span>
                  <span className="font-bold">{selectedReport.postId?.author?.name || 'Unknown User'}</span>
                </p>
